...
namespace autoware_tcl_interface
{
    ...
    struct MessageConversion<MT1, MT2, typename std::enable_if<hasTimingHeader<MT2>::value>::type>
    {
      static void convert(MT1& mt1, MT2& mt2, rclcpp::tcl_node_interfaces::NodeTimingInterface::SharedPtr node_timing)
      {
        ...
        if(timing_header)
        {
          // timing_header->published = true;
          // if (!node_timing->get_timing_propagator()->get_timing_header_arr().empty()) 
          //   node_timing->get_timing_propagator()->get_timing_header_arr().back().published = true;
          mt2.data = mt1;
